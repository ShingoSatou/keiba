...
def _get_database_url(cli_value: str | None) -> str:
    ...
    raise SystemExit("DATABASE_URL is required (env/.env or --database-url).")
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Apply SQL migrations with psycopg.")
    parser.add_argument("--database-url", help="PostgreSQL URL (overrides env/.env DATABASE_URL).")
    parser.add_argument("--dir", default="migrations_v2", help="Migrations directory.")
    ...
    with psycopg.connect(database_url, autocommit=True, row_factory=dict_row) as conn:
        ...
        for migration in migrations:
            ...
            _apply_one(conn, migration, baseline=args.baseline)
