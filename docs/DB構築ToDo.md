...
- [x] **JVRTOpen（0Bxx）の取得オーケストレーション整備** <!-- id: rt-tool-update -->
    ...
    - ✅ `scripts/ops_rt.py`: 当日運用一括取得（0B41/0B14/0B11/0B13/0B17）
...
- [x] **仕様書のステータス更新（反映済）** <!-- id: doc-update -->
    ...
    - `0B11/0B14/0B13/0B17` は現時点では Realtime-only として扱う（必要なら別途方針を追加）。0B16 は JVWatchEvent 前提のため通常運用では使わない。
...
- [ ] **全データのロードテスト**
    ...
    - **注記**:
      ...
      - Realtime-only（0B11/0B14/0B13/0B17）は運用開始以降のデータが揃い次第、別途E2Eで検証する。

### [Future] 将来的な改善（Optional）

- [ ] **当日変更（0B14）の mart 反映（T-5スナップ）**
...
2. Realtime-only（0B11/0B14/0B13/0B17）を投入し、`scripts/ops_t5_dryrun.py --race-date YYYYMMDD` で採用キー/スナップ出力までE2E確認する。
