...
| `scripts_v2/load_to_db.py` | **【主軸】JSONL → DB投入**（`raw.jv_raw` にdedup投入し、`core.*` にupsert。`O1` は `core.o1_header/o1_win/o1_place` へ展開） | `uv run python scripts_v2/load_to_db.py --input-dir data/` |
| `scripts_v2/backfill_o1_place_from_raw_v2.py` | **既存 raw から O1 複勝を再正規化**（`raw.jv_raw` の `0B41/RACE O1` を読み、`core.o1_place` と `core.o1_header` の複勝列を埋める） | `uv run python scripts_v2/backfill_o1_place_from_raw_v2.py --from-date 20160105 --to-date 20260215` |
...
- `DIFF`（`UM/KS/CH`）
- `MING`（`DM/TM`）
- `0B41`（`O1`：単複枠オッズ時系列。`core.o1_win` と `core.o1_place` に展開）
- `0B11`（`WH`：馬体重速報）
...
# 既存 raw から複勝時系列だけを backfill
uv run python scripts_v2/backfill_o1_place_from_raw_v2.py \
  --from-date 20160105 \
  --to-date 20260215 \
  --dataspecs 0B41,RACE
```
...
## 13. `scripts_v2/train_calibrator_v2.py`（Phase 4: Top3確率の校正）

### 何をする？

- Ranker stacking の OOF（例: `data/oof/ranker_stack_convex_oof.parquet`）を入力に、Top3確率 `p_top3` を **walk-forward** で学習/推論します
- 出力の `p_top3` は、後段（ワイド同時確率/バックテスト）の **入力確率** として使用します

### 代表コマンド

```bash
# OOF(2022-2024) → p_top3 の walk-forward OOF(2023-2024) を作る（holdout=2025）
uv run python scripts_v2/train_calibrator_v2.py \
  --input data/oof/ranker_stack_convex_oof.parquet \
  --score-col stack_score \
  --holdout-year 2025 \
  --method logreg \
  --class-weight none \
  ...
  --meta-output models/calibrator_bundle_meta_convex_cw_none.json \
  --force
```

---

## 14. `scripts_v2/eval_calibrator_holdout_v2.py`（Phase 4: holdout校正評価）

### 何をする？

- holdout年（例: 2025）に対して、保存済み校正器（`models/calibrator*.pkl`）を適用し、`p_top3` を出力します
- `--force` なしでは既存出力を上書きしないため、one-shot運用の補助になります
...
uv run python scripts_v2/eval_calibrator_holdout_v2.py \
  --year 2025 \
  --input data/holdout/ranker_stack_2025_compare_preds.parquet \
  --model models/calibrator_convex_cw_none.pkl \
  --preds-output data/holdout/top3_convex_calibrated_2025_cw_none.parquet \
  --metrics-output data/holdout/calibration_convex_2025_metrics_cw_none.json
```

---

## 15. `scripts_v2/backtest_wide_v2.py`（Phase 5: ワイド同時確率 → EV → 資金配分 → バックテスト）

### 何をする？

- 入力: `p_top3` parquet（OOFまたはholdout）
- レースごとに PL+MonteCarlo で `p_wide`（ワイド同時確率）を推定し、`EV = p_wide * odds - 1` で買い目を選抜します
- fractional Kelly + cap（レース/日次）で賭け金を算出し、ROI/MaxDD/月次ROIを集計します

### 主要オプション

- `--ev-threshold`: EV足切り（既定 `0.0`）
- `--min-p-wide`: `p_wide` の確率足切り（既定 `0.0`）
- `--min-p-wide-stage`:
...
uv run python scripts_v2/backtest_wide_v2.py \
  ...
  --years 2023,2024 \
  --holdout-year 2025 \
  --output data/oof/backtest_wide_2023_2024.json \
  --meta-output data/oof/backtest_wide_2023_2024_meta.json \
  --force

# holdout評価（2025を評価に含めるため holdout-year は 2026 へ）
uv run python scripts_v2/backtest_wide_v2.py \
  --input data/holdout/top3_convex_calibrated_2025_cw_none.parquet \
  --years 2025 \
  --holdout-year 2026 \
  --output data/holdout/backtest_wide_2025.json \
  --meta-output data/holdout/backtest_wide_2025_meta.json \
  --force
```

---

## 16. `scripts_v2/sweep_min_p_wide_threshold_v2.py`（Phase 5: `min_p_wide` 閾値スイープ）

### 何をする？

- OOF期間（例: 2023–2024）で `min_p_wide` をグリッドサーチし、ROI最大となる閾値を探索します
- 出力は `data/oof/*.json`（best閾値、全グリッド結果、年別ROIなど）です
...
graph TD
    ...
    OOF2 -->|scripts_v2/sweep_min_p_wide_threshold_v2.py| SWEEP[data/oof/min_p_wide_sweep.json]
