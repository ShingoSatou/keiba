...
v2 のデータ設計・as-of リーク防止・Rolling 年次 CV・OOF 保存の作法を踏襲しつつ、v3 の default main path を次の 3 層に整理する。
...
grouped reference-only meta は比較用 artifact として残すが、PL の default main path では使わない。
...
| **legacy comparison path** | grouped meta / `raw_legacy` / odds calibration は比較用に残すが、default main path の責務は `base -> stacker -> PL` に寄せる |
...
graph TD
    ...
    subgraph パイプライン
        ...
        TMM["train_meta_v3_common.py (legacy)"]
...
### 2.2 legacy / comparison path

- `train_{win,place}_meta_v3.py` は grouped reference-only meta を生成する
- `train_odds_calibrator_v3.py` は odds 由来確率の比較用校正を行う
- これらは `meta_default` / `raw_legacy` profile で比較可能だが、unsuffixed artifact の default route では使わない
