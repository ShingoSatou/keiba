...
4. **PLランキングレイヤ**（uなし）: 線形スコア `s = w^T x` による Plackett-Luce 順位モデル
5. **ワイドROI評価**: Monte Carlo 推定 → Kelly 基準 → バックテスト
...
| **Rolling年次CV** | v2 の `build_rolling_year_folds()` を踏襲し、時系列順序を保証する |
...
| `train_pl_v3.py` | `data/features_v3.parquet` + OOF parquet 群 | `data/oof/pl_v3_oof.parquet`, `data/oof/pl_v3_wide_oof.parquet`, `models/pl_v3_recent_window.joblib` |
