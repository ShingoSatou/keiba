...
- binary = 能力推定
- stack = binary 予測と市場情報（オッズ）の統合
- PL = 統合後 strength の順位分布化
...
| **binary responsibility** | binary default contract は `BINARY_BASE_FEATURES` を中心に構成し、市場オッズは default では入れない |
| **stacker responsibility** | stacker は base 予測と確率化した市場情報、時間差分を統合する |
| **stack-default contract** | PL の default 入力は `p_win_stack`, `p_place_stack`, `place_width_log_ratio`, interaction / race-relative block から作り、raw race context 単体列は入れない |
