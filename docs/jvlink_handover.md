...
| データ取得 (リアルタイム系) | ⚠️ 部分整備 | `scripts/extract_rt_jvlink.py` で単発取得は可能。期間指定バックフィル（racekeyループ）/当日一括取得のオーケストレーションは未整備 |
| パーサー | ✅ 完了 | 仕様書4.9.0.1準拠（オフセット修正済）。DM/TM対応済 |
| DB保存 | ✅ 完了 | FK違反対策実装済。CK/MING対応済 |
| 時系列オッズ | ⚠️ 取得・検証待ち | `0B41` は2003年〜取得可能。取得（バックフィル）→DB投入（`core.o1_*`）のE2E検証が未実施 |
...
> 補足：`0B41/0B42` は **2003年〜現在**で取得可能。一方で、他の 0Bxx（速報/RT）は保持期間・提供有無が異なる可能性があるため、必ず dry-run で確認してください。
