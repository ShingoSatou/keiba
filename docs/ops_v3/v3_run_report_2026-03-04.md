...
- odds系（t10中心）と v2由来のレース条件・相対特徴量（数値列）
...
- `predict_race_v3.py` は PL側で final-odds 特徴量を禁止しているが、現状の win/place モデルは `features_v3` の final/t10 odds列を特徴として含む（厳密な「運用=t10」には再設計が必要）。
