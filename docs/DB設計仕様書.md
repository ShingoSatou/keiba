...
* 対象プロジェクト: `keiba`
* 対象DB: PostgreSQL
* スキーマ定義（一次ソース）: `migrations/0001_init_db.sql`（+ `migrations/*.sql`）
* 最終更新: 2026-02-12
...
以降のカラム定義は `migrations/0001_init_db.sql` に準拠する（本書は「設計書」であり、DDLの一次ソースは `migrations/0001_init_db.sql`）。
...
* DBスキーマ（DDL一次ソース）: `migrations/0001_init_db.sql`
