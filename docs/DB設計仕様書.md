...
* 対象プロジェクト: `keiba`
* 対象DB: PostgreSQL
* スキーマ定義（一次ソース）: `migrations/0001_init_db.sql`, `migrations/0002_init_mart.sql`（+ `migrations/*.sql`）
* 最終更新: 2026-02-15
...
`migrations/0001_init_db.sql` で以下スキーマを作成する。
...
`migrations/0001_init_db.sql` のコメントに従い、`race_id` は以下で一意生成する。
...
以降のカラム定義は DDL（`migrations/*.sql`）に準拠する（本書は「設計書」であり、DDLの一次ソースは `migrations/*.sql`）。
...
* 主キー: `id (BIGSERIAL)`
* 制約:
  * UNIQUE: `(dataspec, rec_id, payload_hash)`（重複排除）
* 主要索引:
...
| `payload_hash` | BYTEA | NO | 重複排除用（sha256(payload)。常に埋める） |
...
## 5F.3 core.rt_mining_dm / core.rt_mining_tm（速報DM/TM履歴: 0B13/0B17）

* 主キー: `(race_id, data_kbn, data_create_ymd, data_create_hm, horse_no)`
* 索引:
  * `idx_rt_mining_dm_race (race_id)` / `idx_rt_mining_tm_race (race_id)`
  * `idx_rt_mining_dm_pick (race_id, data_kbn, data_create_ymd, data_create_hm)`
  * `idx_rt_mining_tm_pick (race_id, data_kbn, data_create_ymd, data_create_hm)`
  * `idx_rt_mining_dm_race_horse (race_id, horse_no)` / `idx_rt_mining_tm_race_horse (race_id, horse_no)`

## 5F.4 core.schedule（YSCH）

* ユニーク: `(race_date, track_code)`
* 索引: `idx_schedule_date (race_date)`

## 5F.5 core.course（COMM: CS）
...
## 5G.2 mart.t5_runner_snapshot（T-5 as-of スナップ）

* 主キー: `(race_id, horse_no, asof_ts)`
* 主要索引:
  * `idx_t5_snapshot_race_date (race_date, race_id)`
  * `idx_t5_snapshot_asof (asof_ts)`
  * `idx_t5_snapshot_feature_set (feature_set)`
* 備考:
  * `feature_set` は backfillable / realtime 等の識別子
  * 採用キー（監査用途）: `o1_announce_mmddhhmi`, `wh_announce_mmddhhmi`, `event_change_keys`, `dm/tm_kbn`, `dm/tm_create_time`, `code_version`

---

## 6. 実装整合性メモ（設計と実装を揃えるための論点）

* `core.runner` のカラムは `migrations/0001_init_db.sql` を一次ソースとする。
  * `scripts/load_to_db.py` が追加カラム（`data_kubun` 等）を参照している場合、スキーマ拡張かローダ修正のいずれかで整合させる。
* `raw.jv_raw` は `payload_hash`（sha256）+ UNIQUE で重複排除する設計（ローダが常に計算して投入）。

---

## 7. 参照

* DBスキーマ（DDL一次ソース）: `migrations/0001_init_db.sql`, `migrations/0002_init_mart.sql`（+ `migrations/*.sql`）
* データ辞書（レコード→カラム）: `docs/データ辞書・正規化仕様書.md`
* データ設計メモ: `docs/data_design.md`
* DBセットアップ: `setup_postgres_multi.sh`（例: `bash setup_postgres_multi.sh keiba_cp`）
